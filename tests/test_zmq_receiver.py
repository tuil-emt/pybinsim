...
def zmq_receiver():
    ...
    zmq_receiver = ZmqReceiver(default_config, soundhandler)
    zmq_receiver.start_listening()
    yield zmq_receiver
    zmq_receiver.close()


@pytest.fixture(scope="session")
def soundhandler(zmq_receiver) -> SoundHandler:
    return zmq_receiver.soundhandler
...
def test_play_command(soundhandler, client):
    # test default ZMQ arguments
    client.send_pyobj(["/pyBinSimPlay", FILEPATH])
    ...
    player_name = 2.1
    client.send_pyobj(["/pyBinSimPlay", FILEPATH, 1,
...
def test_player_control_command(soundhandler, client):
    ...
    client.send_pyobj(["/pyBinSimPlayerControl", FILEPATH, "pause"])
    assert_play_state(player, PlayState.PAUSED)

    client.send_pyobj(["/pyBinSimPlayerControl", FILEPATH, "stop"])
...
def test_player_channel_command(soundhandler, client):
    ...
    client.send_pyobj(["/pyBinSimPlayerChannel", FILEPATH, 1])
...
def test_player_volume(soundhandler, client):
    ...
    client.send_pyobj(["/pyBinSimPlayerVolume", FILEPATH, .5])
    assert_player_volume(soundhandler, FILEPATH, .5)

    client.send_pyobj(["/pyBinSimPlayerVolume", FILEPATH, 1])
...
def test_stop_all_players(soundhandler, client):
    ...
    client.send_pyobj(["/pyBinSimStopAllPlayers"])
...
def test_file_command(soundhandler, client):
    ...
    client.send_pyobj(["/pyBinSimFile", FILEPATH])
